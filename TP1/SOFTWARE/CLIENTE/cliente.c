...
int main( int argc, char *argv[] ) {
	...
	{	
		...
		if(!strcmp( connectt, "connect" ) )
		{
		    ...
		    if(!strcmp(puerto1,"6020\n") /*&& !strcmp(ip,"192.168.1.10") */)
		    {
		    	break;
		    }
		    else
		    {
		    	printf("Comando invalido. Debe ingresar: connect usuer@192.168.1.10:6020 \n");
		    	continue;
		    }	
...
int descarga_archivo(char argv[],char argv1[])
{
    ...
    /** Envio ack de nombre */
    n_udp = sendto( sockfd_udp, (void *)"ack", 4, 0, (struct sockaddr *)&dest_addr_udp, tamano_direccion_udp );
	if( n_udp < 0 ) 
	{
		perror( "Escritura en socket" );
		exit( 1 );
