			...
			{	
				...
				{	
	        		...
	        		/** Si el usaurio esta registrado writee1  es true acceso correcto, de lo contrario writee es true acceso falso */
  				   	if(!strcmp(id_global_user,id_global_pass) && atoi(id_global_pass) > 0 && atoi(id_global_user) > 0 )
				   	{
				   		writee1 = true;
				   	}
				   	else
...
{	/** VARIABLES UTILIZAS PARA NO CONECION */
    ...
    /** Recibo ack de nombre de fichero */
    n_udp = recvfrom( sockfd_udp, buff, 60, 0, (struct sockaddr *)&serv_addr_udp, &tamano_direccion_udp );
	if ( n_udp < 0 ) 
	{
		perror( "lectura de socket" );
		exit( 1 );
    ...
    fclose(f_p);
    /** Recibo ack de tamanio*/
    n_udp = recvfrom( sockfd_udp, tamanio, 60, 0, (struct sockaddr *)&serv_addr_udp, &tamano_direccion_udp );
	if ( n_udp < 0 ) 
	{
		perror( "lectura de socket" );
		exit( 1 );
    ...
    {   
		...
		n_udp = recvfrom( sockfd_udp, (void*)buffer_aux, TAM-1, 0, (struct sockaddr *)&serv_addr_udp, &tamano_direccion_udp );
		if ( n_udp < 0 ) 
		{
			perror( "lectura de socket" );
			exit( 1 );
